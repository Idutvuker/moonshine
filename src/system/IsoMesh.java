...
public class IsoMesh extends Mesh
{
	...
	public IsoMesh(VoxelGrid grid, float isolevel, float scale)
	{
		this.grid = grid;
		...
		material = new BaseMaterial(
				"res/shaders/landscape.vert",
				"res/shaders/landscape.frag",
				vas);

		MarchingCubes(isolevel, scale);
	...
	private void vertInterp(float isolevel, Vector3f p1, Vector3f p2, int val1, int val2, Vector3f dest)
	{
		float mu = (isolevel - val1) / ((float) (val2 - val1));

		mu = Math.max(Math.min(mu, 0.99f), 0.01f);
	...
	private void genIsosurface(float isolevel, ArrayList<Vertex> vertices, List<Integer> indices)
	{
	...
	private void MarchingCubes(float isolevel, float scale)
	{
		...
		genIsosurface(isolevel, vertices, indices);
		...
		for (Vertex vert: vertices)
		{
			...
			verticesBuffer.put(vert.pos.x * scale);
			verticesBuffer.put(vert.pos.y * scale);
			verticesBuffer.put(vert.pos.z * scale);
